...
export const Card : React.FC<Props> = ({
  ...
  if (!currencyData) {
    ...
    const showed_price = (Price*currency_options[Currency?Currency:'USD'].conversion_value).toFixed(2) 
    return (
    <section className='grid grid-cols-2 content-center gap-4 bg-slate-100 shadow-md shadow-black/20 px-8 py-4 rounded-lg'>
      ...
      <h3 className='grid col-span-1 text-center font-bold text-xl'>{ProductName}</h3>
      <h4 className='text-xl text-right text-blue-500 font-bold'><span className='text-sm font-normal'>{currency_options[Currency?Currency:'USD'].prefix}</span> {showed_price}</h4>
